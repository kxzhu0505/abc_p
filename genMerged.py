...
def merge_blif_files(folder_path):
    
    for root, dirs, files in os.walk(folder_path):
        blif_files = [file for file in files if file.endswith(".blif")]
        if blif_files:
            
            merged_blif_path = os.path.join(root, "merged.blif")
            with open(merged_blif_path, "w") as merged_blif:
                merged_blif.write(".model merged\n")
                ...
                merged_blif.write(" ".join(inputs))
                merged_blif.write("\n")
                
                merged_blif.write(".outputs ")
                merged_blif.write(" ".join(outputs))
                merged_blif.write("\n")
